...
Example:
------------------------------------------------------------
    local Promise = NPL.load("(gl)Mod/CodePkuCommon/util/Promise.lua");
-------------------------------------------------------
]]


local Promise = NPL.export();
...
function Promise.new(options)
	...
	d = {
		next = function(self, success, failure)
			local next = Promise.new({success = success, failure = failure, extend = options.extend})
